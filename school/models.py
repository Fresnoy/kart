...
class TeachingArtist(models.Model):
    ...
    presentation_text_fr = models.TextField(
        null=True, blank=True, help_text="General orientation text (not only bio) in FRENCH"
    )
    presentation_text_en = models.TextField(
        null=True, blank=True, help_text="General orientation text (not only bio) in ENGLISH"
    )
...
class StudentApplication(models.Model):
    ...
    campaign = models.ForeignKey(
        StudentApplicationSetup, blank=True, related_name="applications", null=True, on_delete=models.SET_NULL
    )

    current_year_application_count = models.CharField(
        max_length=8, default=None, blank=True, help_text=_("Auto generated field (current year - increment number)")
    )

    identity_card = models.FileField(
        upload_to=make_filepath, null=True, blank=True, help_text="Identity justificative"
    )
    INE = models.CharField(
        max_length=11,
        null=True,
        blank=True,
        help_text="Identifiant National Etudiant (only French student) - 9 numbers + 2 letters",
    )
    # First Candidature
    first_time = models.BooleanField(default=False, help_text="If the first time the Artist's applying")
    last_applications_years = models.CharField(blank=True, max_length=50, help_text=_("Already candidate"))
    # Dates of creation
    ...
    remote_interview_type = models.CharField(
        blank=True, max_length=50, help_text=_("Skype / Gtalk / FaceTime / AppearIn / Other")
    )
    ...
    experience_justification = models.FileField(
        upload_to=make_filepath, null=True, blank=True, help_text="If no master Degree, experience letter"
    )
    # Cursus
    ...
    curriculum_vitae = models.FileField(upload_to=make_filepath, null=True, blank=True, help_text="BIO CV")
    justification_letter = models.FileField(
        upload_to=make_filepath, null=True, blank=True, help_text="Justification / Motivation"
    )
    reference_letter = models.FileField(
        upload_to=make_filepath, null=True, blank=True, help_text="Reference / Recommendation letter"
    )
    free_document = models.FileField(upload_to=make_filepath, null=True, blank=True, help_text="Free document")
    # candidature duo
    ...
    binomial_application_with = models.CharField(
        blank=True, max_length=50, help_text="Name of the binominal artist's candidate with"
    )
    # first and second year project
    considered_project_1 = models.FileField(
        upload_to=make_filepath, null=True, blank=True, help_text="Considered project first year"
    )
    artistic_referencies_project_1 = models.FileField(
        upload_to=make_filepath, null=True, blank=True, help_text="Artistic references for first first year's project"
    )
    considered_project_2 = models.FileField(
        upload_to=make_filepath, null=True, blank=True, help_text="Considered project second year"
    )
    artistic_referencies_project_2 = models.FileField(
        upload_to=make_filepath, null=True, blank=True, help_text="Artistic references for second first year's project"
    )
    doctorate_interest = models.BooleanField(default=False, help_text="Interest in the doctorate")
    # Video
    presentation_video = models.URLField(null=True, blank=True, help_text="Url presentation video Link")
    presentation_video_details = models.TextField(null=True, blank=True, help_text="Details for the video")
    presentation_video_password = models.CharField(blank=True, max_length=50, help_text=_("Password for the video"))
...
class AdminStudentApplication(models.Model):
    ...
    application = models.OneToOneField(
        StudentApplication, related_name="administration", null=True, blank=False, on_delete=models.SET_NULL
    )
    # Administration
    ...
    interview_date = models.DateTimeField(null=True, blank=True, help_text="Date for interview")
    wait_listed_for_interview = models.BooleanField(
        default=False, help_text="Is the candidat wait listed for the Interview"
    )
    position_in_interview_waitlist = models.PositiveSmallIntegerField(
        blank=True, null=True, help_text="Set the position in interview waitlist"
    )
    ...
    position_in_waitlist = models.PositiveSmallIntegerField(
        blank=True, null=True, help_text="Set the position in waitlist"
    )
